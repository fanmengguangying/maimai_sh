...
declare -A targets=(
    ['MaiBot']='maibo_repo_urls'
    ['Adapter']='maibo_adapter_repo_urls'
    ['MaiBot-Napcat-Adapter']='maibo_adapter_repo_urls'
)
declare -A maibo_repo_urls=(
...
declare -A maibo_adapter_repo_urls=(
...
declare -A maibo_adapter_zip_urls=(
...
declare -a MaiBot_Napcat_Adapter_tags=(
    "main"
    "dev"
    "0.4.2"
    "0.2.3"
)
false && Get-MaiMaiRepository "$@"
# Get-MaiMaiRepository 函数说明
# 功能：自动下载 MaiBot 主程序或适配器（支持Git/Zip两种方式）
# 参数说明：
#   $1: 目标仓库 [必填]
#     - "MaiBot"              # 主程序
#     - "Adapter"             # 适配器（同下）
#     - "MaiBot-Napcat-Adapter" # 适配器全称
#   $2: 版本标签 [必填]
#     - 主程序可用版本：main/dev/0.5.8-alpha...0.9.0-alpha
#     - 适配器可用版本：main/dev/0.2.3/0.4.2
#   $3: 下载方式 [可选]
#     - "git" (默认优先)
#     - "zip" (无git时自动切换)
#   $4: 代理URL [可选]
#     - 示例: "http://your-proxy.com/"
#   $5: 自定义目录 [可选]
#     - 默认: "./tmp/MaiM-with-u/[仓库名]"
#
# 使用示例：
#   # 下载主程序main分支
#   repo_path=$(Get-MaiMaiRepository "MaiBot" "main")
#
#   # 强制用zip下载适配器0.4.2版
#   adapter_path=$(Get-MaiMaiRepository "Adapter" "0.4.2" "zip")
#
# 注意事项：
# 1. 需要提前定义：
#    - $Original_User        # 原始用户
#    - $Original_Users_HOME  # 用户家目录
#    - log()函数             # 日志输出
#    - 颜色变量($RED/$GREEN等)
# 2. 依赖命令：
#    - git方式: git
...
Get-MaiMaiRepository() {
    ...
    local target="${1:-}" # MaiBot or Adapter/MaiBot-Napcat-Adapter
    local tag="${2:-}"    # ...
    local way="${3:-git}"
    local proxy_url="${4:-}"
    local dir="${5:-}"
    true ${maibo_repo_urls} ${maibo_zip_urls} ${maibo_adapter_repo_urls} ${maibo_adapter_zip_urls} ${dirNames} # 做给Shellcheck的表面工程
    # 成功git则返回0
    if [[ -z "$way" ]]; then
        command -v git >/dev/null 2>&1 && way="git" || way="zip"
    fi
    [[ "$way" == 'zip' ]] && declare -A -g targets=(
        ['MaiBot']='maibo_zip_urls'
        ['Adapter']='maibo_adapter_zip_urls'
        ['MaiBot-Napcat-Adapter']='maibo_adapter_zip_urls'
    )
    __Check-MaiMaiRepository() {
        local dir="${1:-.}"
        local way2check="${2:-zip}"
        if [[ "$way2check" == "zip" ]]; then
            [[ -d "$dir/src" && -d "$dir/template" && -f "$dir/requirements.txt" ]] && return 0
        else
            bash -c "cd $dir || exit 1;git rev-parse --git-dir 1>/dev/null 2>&1 && exit 0" && return 0
        fi
        return 1
    }
    __Get-Repository() {
        ...
        if [[ "$way2get" == "zip" ]]; then
            ...
            git clone "${url}" "${dir}" >&2 || STATUS=1
        fi
        return "$STATUS"
    }
    local successed_git=0
    __Get-MaiMaiRepository() {
        local way2git="${1:-git}"
        local target="${2:-}"
        local tag="${3:-}"
        local dir="${4:-}"
        local url
        local -n urls="${targets[$target]}"
        local retry=0
        [[ -z "${urls[*]}" ]] && log "空目标，请输入正确的目标(MaiBot/Adapter)" >&2 && return 1
        if [[ -d "${dir}" ]]; then
            chown -R "$Original_User:$Original_User" "${dir}" # 给予权限
            cd "${dir}" >&2 || true                           # Shellcheck别烦我 # 我全程root的脚本需要你教我做事？
            if __Check-MaiMaiRepository . "$1"; then
                log "${GREEN}已存在${dir}，跳过${NC}" >&2
                cd - >&2 || true
                return 0
            else
                cd - >&2 || true
                log "${RED}已存在${dir}，但未检测到git仓库，尝试重新克隆${NC}" >&2
                rm -rf "${dir}"
            fi
        fi
        url="${urls[$tag]}"
        [[ -z "$url" ]] && log "空url，请输入正确的版本" >&2 && return 1
        [[ -n "${proxy_url}" ]] && url="${proxy_url%/}/${url#http*://}"
        mkdir -p "${WORKPATH}/MaiM-with-u"
        mkdir -p "$Original_Users_HOME/.local/MaiM-with-u"
        while ! __Get-Repository "$way2git" "${url}" "${dir}"; do
            ((retry > 3)) && return 1
            log "${RED}克隆失败，正在重试...${NC}" >&2
            ((retry++))
            sleep 2
        done
    }
    [[ -z "$dir" ]] && dir="${WORKPATH}/MaiM-with-u/${dirNames[$target]}"
    case "$dir" in
    */MaiM-with-u/*) ;; # 一般情况下得到的dir为./tmp/MaiM-with-u/(MaiBot/MaiBot-Napcat-Adapter)
    *)
        log "目录校验失败：$dir" >&2
        return 1
        ;;
    esac
    [[ "$dir" == "/" ]] && log "失败！禁止的目录：${dir}" >&2 && return 1
    if ! __Get-MaiMaiRepository "$way" "$target" "$tag" "${dir}"; then
        if [[ -n "${proxy_url}" ]]; then
            log "${RED}git克隆失败，可能是代理问题，请检查代理设置。${NC}" >&2
            log "${YELLOW}尝试使用无代理${NC}" >&2
            Get-MaiMaiRepository "$target" "$tag" "$way" '' "${dir}"
            successed_git=$?
        else
            log "${RED}git克隆失败，请检查网络连接或手动下载。${NC}" >&2
            return "$successed_git"
        fi
    else
        successed_git=0
    fi
    if [[ "$way" == "git" ]]; then
        (cd "${dir}" && git -c advice.detachedHead=false switch "$tag") 2>/dev/null ||
            (cd "${dir}" && git -c advice.detachedHead=false checkout "$tag") 2>/dev/null
    fi
    [[ "$target" == "Adapter" || "$target" == "MaiBot-Napcat-Adapter" ]] && {
        sed -i "s/private_list_type = \"whitelist\"/private_list_type = \"blacklist\"/" "${dir}/template/template_config.toml" >/dev/null 2>&1
        sed -i "s/group_list_type = \"whitelist\"/group_list_type = \"blacklist\"/" "${dir}/template/template_config.toml" >/dev/null 2>&1
        cp "${dir}/template/template_config.toml" "${dir}/config.toml" >/dev/null 2>&1
    }
    if [[ $successed_git == 0 ]]; then printf "%s" "$dir"; fi
    return "$successed_git"
}
