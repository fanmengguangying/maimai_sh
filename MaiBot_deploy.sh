...
__MAIMAI__() {
	cat >"/usr/local/bin/maimai" <<'MAIMAI'
#!/usr/bin/env bash
...
main "$@"
MAIMAI
	chmod +x "/usr/local/bin/maimai"
	echo -e "${GREEN}MaiBot 控制脚本，已生成。${NC}"
}
#########################
...
main(){
    ...
    { 
        ...
        log.info "已设置Bot的QQ号为 ${NAPCAT_QQ:-1145141919810}"
    ...
    }
	{
		...
		python3 -m venv "${HOME}/.local/MaiM-with-u/MaiBotEnv"
		source "${HOME}/.local/MaiM-with-u/MaiBotEnv/bin/activate"
		pip install -i https://pypi.tuna.tsinghua.edu.cn/simple uv
		uv pip install -i https://pypi.tuna.tsinghua.edu.cn/simple --upgrade -r "${HOME}/.local/MaiM-with-u/MaiBot/requirements.txt"
		uv pip install -i https://pypi.tuna.tsinghua.edu.cn/simple --upgrade -r "${HOME}/.local/MaiM-with-u/MaiBot-Napcat-Adapter/requirements.txt"
		log.info "Python依赖安装完成"
		deactivate
	}
	cat <<EOF
安装完成！
请使用以下命令启动 MaiBot 及其组件:
  ${GREEN}maimai start${NC}          # 启动所有服务
  ${GREEN}maimai status${NC}       # 查看服务状态
  ${GREEN}maimai logs napcat${NC}  # 查看 NapCat 日志
  ${GREEN}maimai logs maibot${NC}  # 查看 MaiBot 日志
  ${GREEN}maimai logs adapter${NC} # 查看 Adapter 日志
EOF
}
main
